...
func (s *RequestLogService) List(params *form.RequestLogList) *resources.Collection {
	...
	// 分页查询（只查询列表需要的字段）
	total, collection := model.ListPage(requestLogModel, params.Page, params.PerPage, conditionStr, args, listOptionalParams)
...
func (s *RequestLogService) Detail(id uint) (any, error) {
	...
	if err := requestLog.GetById(requestLog, id); err != nil || requestLog.ID == 0 {
